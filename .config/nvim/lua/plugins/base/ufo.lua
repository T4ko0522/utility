-- Custom markdown fold provider for admonition blocks (:::details, :::message) and headings
---@param bufnr number
...
local function markdownFoldProvider(bufnr)
  ...
  for i, line in ipairs(lines) do
    ...
    -- Handle admonition blocks: :::details, :::message (outside code blocks)
    if not inCodeBlock then
      if line:match("^:::details") or line:match("^:::message") then
        table.insert(blockStack, lineNum)
      elseif line:match("^:::$") and #blockStack > 0 then
        local startLine = table.remove(blockStack)
